...
const train = async (model, data) => {
  ...
  const [trainXs, trainYs] = tf.tidy(() => {
    ...
    const dataSet = d.xs.reshape([TRAIN_DATA_SIZE, 28, 28, 1]);
    const labelSet = d.labels;
  ...
  const [testXs, testYs] = tf.tidy(() => {
    ...
    const dataSet = d.xs.reshape([TEST_DATA_SIZE, 28, 28, 1]);
    const labelSet = d.labels;
    return [dataSet, labelSet];
  });
...
const save = () => {
  ...
  console.log(
    `%c This looks like a(n) ${classNames[pIndex]}!`,
...
const run = async () => {
  ...
  await timeout(10);
  if (!confirm("Start training model?")) return;
